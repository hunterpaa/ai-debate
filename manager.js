...
function addLog(text, type = 'info') {
  ...
  clients.forEach(res => { try { res.write(`data: ${JSON.stringify(entry)}\n\n`); } catch (_) {} });
}

function getStatus() {
  return appProcess && !appProcess.killed ? 'online' : 'offline';
}

function killTree(pid) {
  try { exec(`taskkill /F /T /PID ${pid}`, { windowsHide: true }); } catch {}
}

function freePort(done) {
  exec(
    `for /f "tokens=5" %p in ('netstat -ano ^| findstr ":${APP_PORT} " ^| findstr "LISTENING"') do @taskkill /F /PID %p`,
...
function startApp() {
  ...
  freePort(function() {
    addLog('Iniciando python app.py...', 'info');
    ...
    appProcess.stderr.on('data', d => {
      d.toString().split('\n').map(l => l.trim()).filter(l => l).forEach(l => {
        if (l.includes('address already in use'))
          addLog(`Erro: porta ${APP_PORT} ainda em uso.`, 'error');
        else addLog(l, 'error');
      });
    });
    ...
    appProcess.on('close', code => {
      if (!manualStop && code !== 0 && code !== null) {
        addLog(`Processo encerrado inesperadamente (código ${code}). Reiniciando em 3s...`, 'error');
...
app.post('/start',   (req, res) => { startApp(); res.json({ ok: true }); });
app.post('/stop',    (req, res) => { stopApp();  res.json({ ok: true }); });
app.post('/restart', (req, res) => { stopApp(); setTimeout(startApp, 800); res.json({ ok: true }); });

app.get('/logs', (req, res) => res.send(`<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
<title>Terminal — AI Debate</title>
<style>
...
<title>Gerenciador — AI Debate</title>
<style>
  *{box-sizing:border-box;margin:0;padding:0}
...
app.listen(MANAGER_PORT, () => {
  console.log(`Gerenciador AI Debate -> http://localhost:${MANAGER_PORT}`);
  startApp();
});
